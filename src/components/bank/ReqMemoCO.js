...
//     console.log(messageList);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8081/bank/getMessageList?receiver=4")
      .then((res) => {
  ...
  const saveMemo = () => {
    axios.get(
      `http://localhost:8081/bank/sendMemo?bhno=${bhno}&amount=${amount}&memo=${memo}&messageno=10001`
    );

    // 수임사 화면 리렌더링
    axios
      .post("http://localhost:8081/bank/getHistoryAndSlip", params)
      .then((res) => {
    ...
    <div className="reqMemoCO">
      ...
      <table>
        <tr>
          ...
          <td>
            <input type="text" className="intable" value="신한은행" readOnly />
          </td>
        </tr>
        <tr>
          <td className="modalintitle">일자</td>
          <td>
            <input type="text" className="intable" value={bhdate} readOnly />
          </td>
        </tr>
        <tr>
          <td className="modalintitle">내용</td>
          <td>
            <input type="text" className="intable" value={source} readOnly />
          </td>
        </tr>
        <tr>
          <td className="modalintitle">금액</td>
          <td>
            <input type="text" className="intable" value={amount} readOnly />
          </td>
        </tr>
      </table>
      <br />
